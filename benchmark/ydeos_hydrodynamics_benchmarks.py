...
from ydeos_hydrodynamics.delft import hull_residuary_resistance_2008, \
    due_to_heel, hull_residuary_delta_heeled_ks, \
...
from ydeos_hydrodynamics.sideforce import beta_zero, keel_downwash_on_rudder, \
    lift_curve_slope, sideforce_by_element_fossati, \
    sideforce_production_keuning_verwerft
from ydeos_hydrodynamics.orc import create_interpolation, \
    hull_residuary_resistance_orc_2013
...
from ydeos_hydrodynamics.righting_moment import rm_estimate_gerritsma, \
    rm_orc2013, trm_estimate
from ydeos_hydrodynamics.wetted_area import wetted_area


# List of functions to benchmark
to_profile = (
    (holtrop_mennen, [25.*(1852/3600), 37500, 205., 32., 10., 0.5833, 0.5075,
                      "U", 7381.45, 16., 0.98, 0.45, 0.75,
                      20., 4.0, 10., (50.,), (200.,), (.5,)], {}),
    ...
    (hull_residuary_resistance_2008, [], {'boatspeed': 1., 'Vc': 0.04,
                                          'lwl': 1., 'bwl': 0.2, 'Tc': 0.06,
                                          'Aw': 0.1, 'Cm': 0.75,
                                          'lcb_fraction': 0.52,
                                          'lcf_fraction': 0.53, 'Cp': 0.55}),
    #
    (appendage_friction_coefficient, [], {'speed': 1., 'dimension': 1.,
                                          'thickness_to_chord': 0.06}),
    (appendage_viscous, [], {'boatspeed': 1., 'heel_angle': 10.,
                             'coe': (1., 2., 3.), 'average_chord': 0.1,
                             'thickness_to_chord': 0.06, 'wetted_area': 0.01}),
    (ballast_viscous, [], {'boatspeed': 1., 'heel_angle': 10.,
                           'coe': (1., 2., 3.), 'bulb_length': 0.3,
    ...
    (create_interpolation, [], {'Xs': list(range(40)), 'Ys': list(range(40)),
                                'Zs': list(range(40*40))}),
    (due_to_heel, [], {'boatspeed': 1., 'heel_angle': 10., 'lwl': 1.,
                       'bwl': 0.2, 'Sc': 0.1, 'Tc': 0.06, 'T': 0.4,
                       'coe': None}),
    (froude_number, [1., 1.], {}),
    (hoerner, [0.05], {}),
    (hoerner_streamlined_body, [0.05], {}),
    (hughes, [], {'speed': 1., 'dimension': 1.}),
    (hull_residuary_delta_heeled_ks, [], {'boatspeed': 1., 'heel_angle': 10.,
                                          'Vc': 0.04, 'lwl': 1., 'bwl': 0.2,
                                          'Tc': 0.06, 'lcb_fraction': 0.53}),
    (hull_residuary_resistance_ks_series4, [], {'boatspeed': 1., 'Vc': 0.04,
                                                'lwl': 1., 'bwl': 0.2,
                                                'Aw': 0.1, 'Sc': 0.12,
                                                'lcb_fraction': 0.52,
                                                'lcf_fraction': 0.53,
                                                'Cp': 0.55}),
    (hull_residuary_resistance_la_series3, [], {'boatspeed': 1., 'Vc': 0.04,
                                                'lwl': 1., 'bwl': 0.2,
                                                'Tc': 0.06, 'Aw': 0.1,
                                                'lcb_fraction': 0.52,
                                                'Cp': 0.55}),
    (hull_residuary_resistance_mit, [], {'boatspeed': 1., 'Vc': 0.04, 'lwl': 1.,
                                         'bwl': 0.2, 'Tc': 0.06}),
    (hull_residuary_resistance_orc_2013, [], {'boatspeed': 1., 'Vc': 0.004,
                                              'lwl': 1., 'bwl': 0.2,
                                              'Tc': 0.055}),
    (hull_viscous, [], {'boatspeed': 1., 'lwl': 1., 'Sc': 0.12,
                        'coe': (1., 2., 3.)}),
    ...
    (k_watanabe, [], {'lwl': 1., 'bwl': 0.2, 'Tc': 0.06,
                      'block_coefficient': 0.4}),
    (keel_downwash_on_rudder, [], {'keel_Cl': 0.4,
                                   'keel_effective_aspect_ratio': 3.,
                                   'heel_angle_degrees': 20.}),
    (keel_residuary_delta_heeled_ks, [], {'boatspeed': 1., 'heel_angle': 10.,
                                          'Vc': 0.04, 'Vk': 0.001, 'Tc': 0.06,
                                          'T': 0.4, 'lwl': 1., 'bwl': 0.2,
                                          'coe': (1., 2., 3.)}),
    (keel_residuary_ks, [], {'boatspeed': 1., 'Vc': 0.04, 'Vk': 0.01,
                             'Tc': 0.06, 'T': 0.4, 'lwl': 1., 'bwl': 0.2,
                             'Zcbk': 0.2, 'coe': (1., 2., 3.)}),
    (laminar, [], {'speed': 1., 'dimension': 1.}),
    (lift_curve_slope, [], {'effective_aspect_ratio': 3.,
                            'sweep_back_angle': 10.}),
    (prandtl, [], {'speed': 1., 'dimension': 1.}),
    (reynolds_number, [], {'speed': 1., 'dimension': 1.}),
    (rm_estimate_gerritsma, [], {'boatspeed': 1., 'heel_angle': 10.,
                                 'displacement': 4., 'lwl': 1., 'bwl': 0.2,
                                 'Tc': 0.06, 'Gdwl': 0.2}),
    (rm_orc2013, [], {'heel_angle': 10., 'displacement': 4., 'lwl': 1.,
                      'bwl': 0.2, 'Tc': 0.06}),
    (sideforce_by_element_fossati, [], {'boatspeed': 1., 'heel_angle': 10.,
                                        'leeway_angle': 4., 'rudder_angle': 1.,
                                        'coe': (1., 2., 3.),
                                        'projected_area': 0.05, 'span': 0.4}),
    (sideforce_production_keuning_verwerft, [], {'boatspeed': 1.,
                                                 'heel_angle': 10.,
                                                 'leeway_angle': 4.,
                                                 'rudder_angle': 1.,
                                                 'keel_projected_area': 0.02,
                                                 'keel_span': 0.4,
                                                 'rudder_projected_area': 0.01,
                                                 'rudder_span': 0.2, 'lwl': 1.,
                                                 'bwl': 0.2, 'Tc': 0.06,
                                                 'hull_coe': (1., 2., 3.),
                                                 'keel_coe': (1., 2., 3.),
                                                 'rudder_coe': (1., 2., 3.),
                                                 'keel_sweep_back_angle': 10.,
                                                 'rudder_sweep_back_angle': 0.}),
    (speed_ms, [], {'froude': 0.4, 'lwl': 1.}),
    (transition_x, [], {'speed': 1., 'transition_reynolds_number': 1000000.}),
    (trm_estimate, [], {'trim_angle': 2., 'displacement': 4., 'lwl': 1.,
                        'bwl': 0.2, 'Tc': 0.06, 'Gdwl': 0.2}),
    (wetted_area, [], {'upright_wsa': 0.1, 'bwl': 0.2, 'Tc': 0.06, 'Cm': 0.73,
                       'heel': 20.}),)
