...
def laminar(speed: float,
    ...
    """Calculate the Cf (Friction Coefficient) for a laminar flow
...
def ittc57(speed: float,
           dimension: float,
           kinematic_viscosity: float = KINEMATIC_VISCOSITY_WATER_20C) -> float:
    """ Calculate the Friction Coefficient according to ITTC-57
...
def hughes(speed: float,
           dimension: float,
           kinematic_viscosity: float = KINEMATIC_VISCOSITY_WATER_20C) -> float:
    """Calculate the Friction Coefficient) according to the Hughes friction line
    The Hughes friction line replaced the ittc57 friction line in the 2013 ORC VPP
...
def prandtl(speed: float,
    ...
    """Calculate the friction coefficient according to Prandtl formula (transitional friction line)
    This is best adapted for a situation with laminar and turbulent flow on the same body.

    speed : The flow speed [m/s]
    dimension : Characteristic dimension [m]
    transition_reynolds_number : the Reynolds number at which the transition occurs (default 5e5)
    kinematic_viscosity defaults to kinematic viscosity of water at 20°C : 1,004*10E-6 [m**2/s]
    turbulent_friction_line : the function used to compute the turbulent Cf (ittc57, hughes)
    ...
    x_transition = transition_x(speed, transition_reynolds_number, kinematic_viscosity)
    if x_transition >= dimension:
        ...
        cf_turb_xtr = turbulent_friction_line(speed, x_transition, kinematic_viscosity)
...
def appendage_friction_coefficient(speed: float,
                                   ...
                                   kinematic_viscosity: float = KINEMATIC_VISCOSITY_WATER_20C) -> float:
    """See page 53 of 2013 ORC VPP - Computes the friction coefficient for an appendage
...
def bulb_friction_coefficient(speed: float,
                              dimension: float,
                              kinematic_viscosity: float = KINEMATIC_VISCOSITY_WATER_20C) -> float:
    """Computes the friction coefficient for a bulb - See page 53 of 2013 ORC VPP
